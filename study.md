...
读 README.md — 一句话定位：一个"人在环内（HITL）"的客服 agent，邮件进来 → AI 起草 → 高风险/低置信度时停在飞书等人审批 → 人才授权发。
...
nodes.py — 各节点函数（分类/起草/发审批卡片/interrupt）
graph.py — 节点怎么连成图、interrupt 在哪
email_listener.py — 你刚看的 IMAP 轮询（已懂）
feishu_handler.py — 飞书审批卡片回调；slack_handler.py 是兼容回退
...
第 6 步 · 安全与评测（按需）
docs/threat_model.md — 能力隔离（Read 不能发、Email 不能发审批卡片等）、PII 进出
